...
echo username: $(kubectl -n jenkins jenkins exec $POD -- env | grep ADMIN_USER | sed 's/.*=//')
echo password: $(kubectl -n jenkins jenkins exec $POD -- env | grep ADMIN_PASSWORD | sed 's/.*=//')
